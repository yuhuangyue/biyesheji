...
# 数据参数
MODEL_DIR = 'model_cnn'  # inception-v3模型的文件夹
MODEL_FILE = 'tensorflow_inception_graph.pb'  # inception-v3模型文件名
CACHE_DIR = 'data\\tmp\\bottleneck'  # 图像的特征向量保存地址
INPUT_DATA = 'data\\dataset'  # 图片数据文件夹
...
#定义新模型地址
pb_file_path = "basketball_model.pb"
#类的标签名
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category,
    ...
    if not os.path.exists(bottleneck_path):
        ...
        image_forme = Image.open(image_path)
        if image_forme.format != "JPEG":
            get_value = False
            bottleneck_values = 0
            print("********************小坏蛋！",image_forme.format)
            print(image_path)
...
def main(_):
    ...
    with tf.name_scope('full_connect_layer2') as scope:
        ...
        final_tensor = tf.nn.softmax(logits_local2)  # 直接就softmax输出le
    ...
    loss_scalar = tf.summary.scalar('loss', cross_entropy_mean)
    ...
    with tf.Session() as sess:
        ...
        # 下面是输出测试的结果
        ans = sess.run(logits_local,feed_dict={bottleneck_input: test_bottlenecks, ground_truth_input: test_ground_truth})
       # print ("mid-layer-----",ans)
        ...
        csvfile = open('basketball_test.csv','w',newline='')
        writer = csv.writer(csvfile)
        for i in range(len(test_ground_truth)):
            row = []
            for pos11 in range(250):
            	#writer.writerow(i,pos11,float(ans[i][pos11-1]))
                row.append(ans[i][pos11-1])

            for pos2 in range(n_classes):
                #writer.writerow(i,pos1+pos2,float(test_ground_truth[i][pos2-1]))
                row.append(test_ground_truth[i][pos2-1])
            writer.writerow(row)
