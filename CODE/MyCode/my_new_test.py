...
# 数据参数
MODEL_DIR = 'C:\\Users\\56891\\Desktop\\bird\\model'  # inception-v3模型的文件夹
MODEL_FILE = 'tensorflow_inception_graph.pb'  # inception-v3模型文件名
CACHE_DIR = 'C:\\Users\\56891\\Desktop\\bird\\bottleneck'  # 图像的特征向量保存地址
INPUT_DATA = 'C:\\Users\\56891\\Desktop\\bird\\image'  # 图片数据文件夹
...
def main(_):
    ...
    # 定义一层全连接层解决新的图片分类问题
    with tf.name_scope('final_training_ops') as scope:
    ...
    with tf.variable_scope('softmax_linear') as scope:
        weights = tf.Variable(tf.truncated_normal([n_classes, n_classes], stddev=0.1))
        biases = tf.Variable(tf.zeros([n_classes]))
        softmax_linear = tf.add(tf.matmul(logits_local, weights), biases, name=scope.name) # 一样的，相加 ；tf.matmul是矩阵乘法
    """    

    # 定义交叉熵损失函数
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
        logits=logits_local, labels=ground_truth_input)
    ...
    with tf.Session() as sess:
       ...
       # logpath=r'/Users/gaoyang5/Downloads/YUhuangyue/inception/graph'
...
if __name__ == '__main__':
    tf.app.run()
"""
    for filename in os.listdir(INPUT_DATA):  # listdir的参数是文件夹的路径

        print(filename)  # 此时的filename是文件夹中文件的名称
        filename = INPUT_DATA +'\\' + filename
        for image_path in os.listdir(filename):  # listdir的参数是文件夹的路径


            image_path = filename +'\\' +image_path
           # print(image_path)  # 此时的filename是文件夹中文件的名称
